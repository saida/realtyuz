  ...
  module Acts #:nodoc:
    module Commentable #:nodoc:
      ...
      module SingletonMethods
        ...
        # the mixin commentable type written by a given account.  
        # This method is NOT equivalent to Comment.find_comments_for_account
        def find_comments_by_account(account) 
          commentable = ActiveRecord::Base.send(:class_name_of_active_record_descendant, self).to_s
          
          Comment.find(:all,
            :conditions => ["account_id = ? and commentable_type = ?", account.id, commentable],
            :order => "created_at DESC"
          )
        end
      end
