   ...
   #
    class Graphic
      ...
      COLORS = ['AFD8f6', '8E468E', '588526', 'B3A000', 'B2FF66',
                ...
                '66FF33', '555555', 'FFFF00', '669966', 'FF3300',
      ...
      def to_xml
        ...
        if ["area2d"].include? self.c_type
          ...
          escaped_xml = g_xml.graph(options) do
            self.data.each do |data| #{ |k,v|
              g_xml.set :value => data[1], :name => data[0]
            end  #}
          end
        elsif !['msline', 'mscol2d', 'msbar2d', 'mscol3d'].include?(self.c_type)
          escaped_xml = g_xml.graph(options) do
            self.data.each do |data| #{ |k,v|
              g_xml.set :value => data[1], :name => data[0], :color => self.rand_color
            end  #}
          end
        else
